...
public class Model { //Kender ikke andre objekter
    ...
    return db.query("SELECT t.id AS id, h1.name AS fromport, h2.name AS toport, v.id AS vessel, TOTAL(f.containers) AS flow, v.capacity" +
            " FROM transport t " +
            "INNER JOIN vessel v ON t.vessel = v.id " + //vessel id column joins the transport table, on the condition that t.vessel = v.id, which means that the vessel column in the transport table must be equal to the id column in the vessel table.
            "INNER JOIN habour h1 ON t.fromhabour = h1.id " +
            "INNER JOIN habour h2 ON t.tohabour = h2.id " +
            "LEFT OUTER JOIN flow f ON t.id = f.transport " + //flow.transport column joins transport.id column if the id is equal. If no matching rows in the right table, NULL values are returned in the right table's columns.
